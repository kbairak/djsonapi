...
- [ ] **Support Flask/FastAPI** — extract framework-agnostic core so djsonapi works outside Django
- [ ] **More SDK targets** — generate Go clients from the same API introspection
- [ ] **Non-JSON:API endpoints in OpenAPI spec** — register arbitrary routes (e.g. `/articles/{id}/publish`) via `@api.endpoint()` and include them in the generated OpenAPI doc
